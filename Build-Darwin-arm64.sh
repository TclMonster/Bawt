...
TCLKIT="./tclkit-Darwin-arm64"
NUMJOBS=4
ACTION="--${ACTION}"

BAWTOPTS="--rootdir ${OUTROOTDIR} --architecture ${ARCH} --numjobs ${NUMJOBS}"
EXCLOPTS="--exclude mupdf --exclude tclMuPdf --exclude expect --exclude vectcl --exclude libffi --exclude Ffidl --exclude cffi"
LIBOPTS="--copt tcllib Critcl=OFF"

# Build all libraries as listed in Setup file.
${TCLKIT} Bawt.tcl ${BAWTOPTS} ${EXCLOPTS} ${LIBOPTS} ${ACTION} ${SETUPFILE} ${TARGETS}
